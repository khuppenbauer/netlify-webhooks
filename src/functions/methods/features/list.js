...
const filteredResult = async (event) => {
  let result;
  let collectionCount;
  await Feature.estimatedDocumentCount(function (err, count) {
  ...
  const totalCount = await Feature.count(search);
  ...
  try {
    result = await Feature.find(search, null, options);
  } catch (err) {
    return {
      statusCode: 400,
      headers: {
...
const aggregationResult = async (event) => {
  ...
  if (type === 'aggregate') {
    try {
      result = await Feature.aggregate(pipeline);
    } catch (err) {
      return {
        statusCode: 400,
        headers: {
