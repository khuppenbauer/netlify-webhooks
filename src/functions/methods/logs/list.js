...
const filteredResult = async (event) => {
  ...
  try {
    result = await Log.find(filter, null, options);
  } catch (err) {
    return {
      statusCode: 400,
      headers: {
...
const aggregationResult = async (event) => {
  ...
  if (type === 'aggregate') {
    try {
      result = await Log.aggregate(pipeline);
    } catch (err) {
      return {
        statusCode: 400,
        headers: {
